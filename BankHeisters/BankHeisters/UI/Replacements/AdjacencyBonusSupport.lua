...
local tDistrictsWithAdjacencyBonuses:table = {};

for row in GameInfo.District_Adjacencies() do
	local districtIndex = GameInfo.Districts[row.DistrictType].Index;
	if (districtIndex ~= nil) and tDistrictsWithAdjacencyBonuses[districtIndex] ~= true then
		tDistrictsWithAdjacencyBonuses[districtIndex] = true;
	end
end
...
function GetAdjacentYieldBonusString( eDistrict:number, pkCity:table, plot:table )
	...
	if (GameInfo.Districts[eDistrict].OnePerCity == false) and (tDistrictsWithAdjacencyBonuses[eDistrict] == nil) then
